...
export const TodoList = ({ statuses }: TodoListProps) => {
  const [parent] = useAutoAnimate<HTMLUListElement>()
  const { data: todos = [], refetch } = api.todo.getAll.useQuery({
    statuses,
  })

  const handleStatusChange = (todoId: number, checked: boolean | undefined) => {
    updateTodoStatus({
      todoId,
  ...
  return (
    <ul ref={parent} className="grid grid-cols-1 gap-y-3">
      {todos.map((todo) => (
        <li key={todo.id}>
          <div
            className={`flex items-center rounded-12 border border-gray-200 px-4 py-[11px] shadow-sm ${
          ...
          >
            ...
            <button
              ...
              className="ml-auto p-1 px-0 text-gray-500 hover:text-gray-800 "
            >
